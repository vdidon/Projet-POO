...
static void text_output(int x, int y, const WCHAR* str)
{ 
    select_font();
    if (text_color != color) { 
	text_color = color;
    ...
    if (bgiemu_handle_redraw || visual_page != active_page) { 
        TextOut(hdc[1], x, y, str, static_cast<int>(wcslen(str)));
    }
    if (visual_page == active_page) { 
        TextOut(hdc[0], x, y, str, static_cast<int>(wcslen(str)));
    } 
}
...
int textheight(const WCHAR* str)
{
    SIZE ss;
    select_font();
    GetTextExtentPoint32(hdc[0], str, static_cast<int>(wcslen(str)), &ss);
    return ss.cy;
}

int textwidth(const WCHAR* str)
{
    SIZE ss;
    select_font();
    GetTextExtentPoint32(hdc[0], str, static_cast<int>(wcslen(str)), &ss);
...
void outtextxy(int x, int y, const WCHAR* str)
{
			 ...
			 WPARAM wParam, LPARAM lParam)
{
	...
	break;
      case WM_CHAR:
	kbd_queue.put(static_cast<char>(wParam));
	break;
	  case WM_LBUTTONUP:
	  case WM_MBUTTONUP:
	  case WM_RBUTTONUP :
